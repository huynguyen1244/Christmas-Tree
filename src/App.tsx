...
const PhotoOrnaments = ({ state, focusedIndex, photoZoom }: { state: 'CHAOS' | 'FORMED', focusedIndex: number | null, photoZoom: number }) => {
  ...
  // Vị trí ảnh khi focus (trước camera - camera ở z=60, y=8)
  const focusPos = useMemo(() => new THREE.Vector3(0, 5, 55), []);
...
const PhotoModal = ({ photoPath, startPosition, onClose }: {
  photoPath: string | null;
  startPosition: { x: number; y: number } | null;
  onClose: () => void
}) => {
  if (!photoPath || !startPosition) return null;

  // Tính toán vị trí bắt đầu tương đối so với center
  ...
  return (
    ...
    >
      <div style={{
        ...
        '--start-x': `${startX}px`,
        '--start-y': `${startY}px`,
        animation: 'photoFlyIn 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)'
        ...
        <div style={{
          ...
          height: '20px',
          background: 'radial-gradient(circle, #FFD700 0%, transparent 70%)',
          animation: 'sparkle 1s ease-out 0.1s forwards',
          borderRadius: '50%'
        }} />
        <div style={{
          position: 'absolute',
        ...
        <div style={{
          ...
          height: '20px',
          background: 'radial-gradient(circle, #FFD700 0%, transparent 70%)',
          animation: 'sparkle 1s ease-out 0.3s forwards',
          borderRadius: '50%'
        }} />
        <div style={{
          position: 'absolute',
      ...
      <style>{`
        ...
        @keyframes photoFlyIn {
          0% { 
            transform: translate(var(--start-x), var(--start-y)) scale(0.1) rotate(-15deg); 
            opacity: 0;
          }
          30% {
            opacity: 1;
